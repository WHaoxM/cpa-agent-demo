 ...
 * @returns 百分比
 */
export function calculatePercentage(value: number, total: number, digits: number = 1): number {
  if (total === 0) return 0
  return Math.round((value / total) * 100 * Math.pow(10, digits)) / Math.pow(10, digits)
}
