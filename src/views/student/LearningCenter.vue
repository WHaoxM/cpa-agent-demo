...
const prereqOption = computed(() => {
  ...
  const softNodeFill = isNight.value ? 'rgba(255,255,255,0.10)' : 'rgba(17,24,39,0.06)'
  const softNodeBorder = isNight.value ? 'rgba(255,255,255,0.42)' : 'rgba(17,24,39,0.18)'
  return {
    ...
    tooltip: {
      ...
      borderWidth: 0,
      backgroundColor: isNight.value ? 'rgba(17, 24, 39, 0.86)' : 'rgba(255, 255, 255, 0.94)',
...
const palette = computed(() => {
  const grid = isNight.value ? 'rgba(148, 163, 184, 0.22)' : 'rgba(17, 24, 39, 0.08)'
  const brand = themePalette.value.primary || (isNight.value ? '#7C3AED' : '#409EFF')
  const brand2 = themePalette.value.accent || (isNight.value ? '#22D3EE' : '#10B981')
  return {
    text: themePalette.value.text || (isNight.value ? '#E5E7EB' : '#111827'),
    muted: themePalette.value.textMuted || (isNight.value ? '#A1A1AA' : '#4B5563'),
    grid,
    brand,
    brand2,
  }
...
const forceOption = computed(() => {
  ...
  return {
    ...
    tooltip: {
      ...
      borderWidth: 0,
      backgroundColor: isNight.value ? 'rgba(17, 24, 39, 0.82)' : 'rgba(255, 255, 255, 0.92)',
      textStyle: {
        color: palette.value.text,
      },
      extraCssText: `backdrop-filter: blur(10px); border-radius: 12px; padding: 10px 12px;`,
...
const sankeyOption = computed(() => {
  return {
    ...
    tooltip: {
      ...
      borderWidth: 0,
      extraCssText: `backdrop-filter: blur(10px); border-radius: 12px; padding: 10px 12px; color: ${palette.value.text};`,
    },
    series: [
      {
        type: 'sankey',
        ...
        itemStyle: {
          borderWidth: 1,
          borderColor: isNight.value ? 'rgba(255,255,255,0.18)' : 'rgba(17,24,39,0.10)',
        },
        label: {
          color: palette.value.text,
          fontWeight: 700,
...
<template>
  <div class="learning-center page page--compact">
    <section class="career">
      ...
      <div class="career-grid">
        <div class="panel panel--chart panel--force">
          ...
          <div class="force-wrap">
            ...
            <div v-if="prereqOption" class="prereq-float">
    ...
    <section class="course-block">
        ...
        <div class="topbar">
          <div class="topbar__left">
            <div class="search-box">
              <Icon icon="fluent-emoji:magnifying-glass-tilted-left" class="search-icon" />
              <input
                v-model="searchQuery"
                type="text"
                placeholder="搜索课程..."
          ...
          <div class="topbar__right">
            <button class="btn-icon" @click="refreshData" title="刷新">
              <Icon icon="fluent-emoji:counterclockwise-arrows" />
            </button>
      ...
      <div v-if="filteredCourses.length > 0" class="courses-grid-v2">
        ...
        >
          <div class="card-image-wrap">
            ...
            >
              <Icon
                :icon="courseStore.isCourseFavorite(course.id) ? 'fluent-emoji:glowing-star' : 'fluent-emoji:star'"
              />
            </button>
          ...
          <div class="card-body">
            ...
            <div class="card-meta">
              <span class="meta-item">
                <Icon icon="fluent-emoji:teacher" />
                {{ course.teacherName }}
              </span>
              <span class="meta-item">
      ...
      <div v-else class="empty-state-v2">
        <Icon icon="fluent-emoji:empty-nest" class="empty-icon-v2" />
        <p>暂无相关课程</p>
...
.career-head {
  ...
  background: radial-gradient(
      900px 260px at 10% 0%,
      color-mix(in srgb, var(--primary-100) 14%, transparent 86%),
      transparent
    ),
...
[data-theme='night'] .prereq-float {
  background: color-mix(in srgb, var(--bg-200) 78%, #000000 22%);
  border-color: color-mix(in srgb, var(--bg-300) 70%, transparent 30%);
...
[data-theme='night'] .prereq-float__head {
  border-bottom-color: color-mix(in srgb, var(--bg-300) 70%, transparent 30%);
}
...
.card-ribbon {
  ...
  width: 84px;
  height: 22px;
  background: linear-gradient(90deg, color-mix(in srgb, var(--primary-100) 80%, #ffffff 20%), color-mix(in srgb, var(--accent-100) 70%, var(--primary-100) 30%));
  opacity: 0.9;
  transform: rotate(-12deg);
  border-radius: 999px;
  filter: blur(0px);
  box-shadow: 0 8px 18px color-mix(in srgb, var(--primary-100) 18%, transparent);
  z-index: 1;
  pointer-events: none;
}
